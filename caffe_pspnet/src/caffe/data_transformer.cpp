...
void DataTransformer<Dtype>::TransformImgAndSegOnline(
    ...
    //    for(int i=0;i< cv_img_seg[0].rows;i++)
    //        for(int j=0;j< cv_img_seg[0].cols;j++)
    //        {
    //            unsigned char* mask_pixel = cv_img_seg[1].data + i* cv_img_seg[1].step[0] + j* cv_img_seg[1].step[1];
    //            if(!mask_pixel[0])
    //            {
    //                unsigned char* img_pixel = cv_img_seg[0].data + i* cv_img_seg[0].step[0] + j* cv_img_seg[0].step[1];
    ...
    //    cv::imshow("image_new",cv_img_seg[0]);
    //    cv::waitKey(1);

//    bool has_rand_brightness = true;
//    bool has_rand_contrast = true;
//    bool has_rand_saturation = true;
//    bool has_rand_hue = true;

    bool has_gaussian_blur = true;
    bool has_rand_rotate = true;
    bool has_rand_scale = true;

//    std::cout << param_.has_rand_brightness() << "  " <<;
//    param_.has_rand_contrast() << "  " <<
//             param_.has_rand_saturation() << "  " <<
//                                  param_.has_rand_hue() << "\n";
...
//    int value_shift;
//    int apply_brightness = rng(2);
//    if(has_rand_brightness && apply_brightness)
//    {
//        value_shift = rng.uniform(0,rand_brightness);
//        if(rng(2)) value_shift = -value_shift;

//        cv::Mat hsv;
//        cv::cvtColor(cv_img_seg[0],hsv,CV_BGR2HSV);

//        for(int i=0;i<hsv.rows;i++)
//            for(int j=0;j<hsv.cols;j++)
//            {
//                unsigned char* pixel = hsv.data + i * hsv.step[0] + j * hsv.step[1];


//                if((pixel[2]+value_shift)>0 && (pixel[2]+value_shift) <256)
//                    pixel[2] += value_shift;

//            }

//        cv::cvtColor(hsv,cv_img_seg[0],CV_HSV2BGR);
//    }

    // gaussian blur
    int smooth_param = 3;
    int apply_smooth = rng(2);
    if ( has_gaussian_blur && apply_smooth ) {
        smooth_param += 2*rng(2);
        cv::GaussianBlur(cv_img_seg[0], cv_img_seg[0], cv::Size(smooth_param,smooth_param),0);
    }

    std::vector<float> rand_rotate(2);
    std::vector<float> rand_scale(2);
    ...
    if (has_rand_rotate && apply_rotation)
    {

        float rand_rotate_min = rand_rotate[0];
        float rand_rotate_max = rand_rotate[1];
        ...
        cv::warpAffine(cv_img_seg[1], cv_img_seg[1], r, cv::Size(cv_img_seg[1].cols, cv_img_seg[1].rows),cv::INTER_NEAREST);
    }
